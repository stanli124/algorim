 ...
 *一般情况下，罗马数字中小的数字在大的数字右边。但也存在特例，4写做IV不写作IIII。小数在大数坐标表示减法，小数在大数右边表示加法；
 *
...
//    }
    ...
    //字符串从左到右来，如果当前字符代表的值不小于其右边，就加上该值；否则就减去该值。以此类推到最左边的数
    public static int romanToInt(String s){
        HashMap<String, Integer> map = new HashMap();  //用来取得罗马字符对应的数值；
        map.put("I",1);
        map.put("V",5);
...
//            if (map.get(String.valueOf(s.charAt(i))) >= map.get(String.valueOf(s.charAt(i+1)))) sum = sum + map.get(String.valueOf(s.charAt(i)));
//            else sum-=map.get(String.valueOf(String.valueOf(s.charAt(i))));

            if (map.get(c1) >= map.get(c2)) sum = sum + map.get(c1);
            else sum-=map.get(String.valueOf(c1));
